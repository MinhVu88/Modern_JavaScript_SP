class Forecast {
    constructor() {
        this.api_key = 'Juve6kA4rt21TzXeH6mXqfVF5cRYrCwE';
    ...
    async updateCity(city) {
        const city_data = await this.getCity(city);

        const weather_data = await this.getWeather(city_data.Key);

        return {city_data, weather_data};
    }

    async getCity(city) {
        const query_params = `?apikey=${this.api_key}&q=${city}`;

        const response = await fetch(this.city_url + query_params);
    ...
    async getWeather(locationKey) {
        const query_params = `${locationKey}?apikey=${this.api_key}`;

        const response = await fetch(this.weather_url + query_params);
